...
template<typename T>
void templateSpaceToPlane(
  const std::vector<T> & intrinsic_params, const Eigen::Matrix<T, 3,
  1> & P_c, Eigen::Matrix<T, 2, 1> & p)
{
...
template<typename T>
void templateLiftProjective(
  const std::vector<T> & intrinsic_params, const Eigen::Matrix<T, 2,
  1> & p, Eigen::Matrix<T, 3, 1> & P)
{
  T k1 = intrinsic_params[0];
  T k2 = intrinsic_params[1];
  T p1 = intrinsic_params[2];
...
struct LidarPlaneNormFactor
{
  ...
  LidarPlaneNormFactor(
    const Eigen::Vector3d & curr_point_, const Eigen::Vector3d & plane_unit_norm_,
...
public:
  ...
  PointPosition33FactorQT(
    const Eigen::Vector3d & map_point_,
    const Eigen::Vector3d & observed_point_, const float & weight_)
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ReprojectionPoint32Factor1(
    std::vector<double> m_intrinsic_params_,
    const Eigen::Vector3d & map_point_,
  ...
  static ceres::CostFunction * Create(
    const std::vector<double> m_intrinsic_params_,
    const Eigen::Vector3d & map_point_,
    const Eigen::Vector2d & observed_point_)
  {
    return new ceres::AutoDiffCostFunction<
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ReprojectionPoint32Factor3new(
    std::vector<double> m_intrinsic_params_, const Eigen::Vector2d & observed_point_,
  ...
  static ceres::CostFunction * Create(
    const std::vector<double> m_intrinsic_params_, const Eigen::Vector2d & observed_point_,
    const Eigen::Vector3d & first_track_point_ /*, const Eigen::Matrix<double, 7,1>& first_depth_pose_*//*,const Eigen::Quaterniond& first_depth_q_, const Eigen::Vector3d& first_depth_t_*/)
  {
    return new ceres::AutoDiffCostFunction<
      ...
      new ReprojectionPoint32Factor3new(
        m_intrinsic_params_, observed_point_,
        first_track_point_ /*, first_depth_pose_*//*, first_depth_q_, first_depth_t_*/));
  }
