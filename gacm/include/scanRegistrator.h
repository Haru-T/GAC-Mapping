...
private:
    ...
    pcl::PointCloud<PointT> & cloud_out, float thres)
  {
    ...
    for (size_t i = 0; i < cloud_in.points.size(); ++i) {
      if (cloud_in.points[i].x * cloud_in.points[i].x + cloud_in.points[i].y *
        cloud_in.points[i].y + cloud_in.points[i].z * cloud_in.points[i].z < thres * thres)
      {
        continue;
      }
...
public:
  ...
  void markOccludedPoints()
  {
    ...
    for (int i = 5; i < cloudSize - 6; ++i) {
      ...
      int columnDiff =
        std::abs(int(laserRangeCloud->points[i + 1].y - laserRangeCloud->points[i].y));
