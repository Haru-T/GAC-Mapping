...
public:
  ...
  ros::Publisher pubLaserCloudSurround, pubLaserCloudMap, pubLaserCloudFullRes, pubOdomAftMapped,
    pubOdomAftMappedHighFrec, pubLaserAfterMappedPath, pubMatchSurround, pubMatchPose;
  ...
  void initParameters(
    const int & robot_id_, const int & submap_id_, const bool & is_drone_,
    const double & base_stamp_)
  {
    robot_id = robot_id_;
  ...
  SubMap(
    const int & robot_id_, const int & submap_id_, const bool & is_drone_,
    const Eigen::Quaterniond & q_world_base_, const Eigen::Vector3d & t_world_base_,
    const Eigen::Quaterniond & q_odom_last_, const Eigen::Vector3d & t_odom_last_,
    const double & base_stamp_, ros::NodeHandle nh_)
  : robot_id(robot_id_), submap_id(submap_id_), is_drone(is_drone_), nh(nh_)
  {
    allocateMemory();
    access_status = submap_id;
  ...
  SubMap(
    std::string map_path, const int & robot_id_, const int & submap_id_,
    const bool & is_drone_, ros::NodeHandle nh_)
  : robot_id(robot_id_), submap_id(submap_id_), is_drone(is_drone_), nh(nh_)
  {
    //load from file
    allocateMemory();
  ...
  void setActive(int idx)
  {
    if (access_status == -1) {
      ...
      if (pcl::io::loadPCDFile<PointType>(
          CACHE_PATH + ss.str() + "corner.pcd", *laserCloudMapCorner) == -1)
      {
      ...
      if (DEBUG) {
        std::cout << "Load done, start pushing cloud with corner " <<
          laserCloudMapCorner->points.size() << " and surf " << laserCloudMapSurf->points.size() <<
          "\n";
      }
      for (int i = 0; i < laserCloudMapCorner->points.size(); i++) {
        ...
        {
          int cubeInd = cubeI + laserCloudWidth * cubeJ + laserCloudWidth * laserCloudHeight *
            cubeK;
  ...
  void loadMapFromFile(std::string map_path)
  {
    ...
    // fs.open(map_path+parameters_file, std::ios::out);
    fs >> q_world_base.w() >> q_world_base.w() >> q_world_base.x() >> q_world_base.y() >>
    q_world_base.z();
    fs >> t_world_base.x() >> t_world_base.y() >> t_world_base.z();
    fs >> q_base_curr.w() >> q_base_curr.w() >> q_base_curr.x() >> q_base_curr.y() >>
    q_base_curr.z();
    fs >> t_base_curr.x() >> t_base_curr.y() >> t_base_curr.z();
    fs >> q_last_curr.w() >> q_last_curr.w() >> q_last_curr.x() >> q_last_curr.y() >>
    q_last_curr.z();
    fs >> t_last_curr.x() >> t_last_curr.y() >> t_last_curr.z();
    fs >> q_odom_last.w() >> q_odom_last.w() >> q_odom_last.x() >> q_odom_last.y() >>
    q_odom_last.z();
    fs >> t_odom_last.x() >> t_odom_last.y() >> t_odom_last.z();
    fs >> q_world_curr.w() >> q_world_curr.w() >> q_world_curr.x() >> q_world_curr.y() >>
    q_world_curr.z();
    fs >> t_world_curr.x() >> t_world_curr.y() >> t_world_curr.z();
  ...
  std::shared_ptr<SubMap> createNewSubMap()
  {
    ...
    next_submap_ptr =
      ...
      new SubMap(
        robot_id, submap_id + 1, is_drone, q_world_curr,
        t_world_curr, q_odom_last, t_odom_last, timeLaserOdometry, nh));
    ...
    for (int i = 0; i < cornerCloudSurround->size(); i++) {
      ...
      if (pointSel.x * pointSel.x + pointSel.y * pointSel.y + pointSel.z * pointSel.z >
        KEEP_SQURE_DISTANCE)
      {
        continue;
      }
    ...
    nav_msgs::Odometry::ConstPtr odometryBufFrontPtr)
  {
    ...
    if (frameCount % 5 == 0) {
        ...
        for (int j = 0; j < laserCloudHeight; j++) {
          for (int k = 0; k < laserCloudDepth; k++) {
            ...
            pcl::PointCloud<PointType>::Ptr laserCloudCubeCornerPointer =
              laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
                k];
            ...
            for (; i >= 1; i--) {
              laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
                k] =
                laserCloudCornerArray[i - 1 + laserCloudWidth * j + laserCloudWidth *
                  laserCloudHeight * k];
              laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
                k] =
                laserCloudSurfArray[i - 1 + laserCloudWidth * j + laserCloudWidth *
                  laserCloudHeight * k];
            }
            laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
              k] =
        ...
        for (int i = 0; i < laserCloudWidth; i++) {
          for (int k = 0; k < laserCloudDepth; k++) {
            ...
            for (; j >= 1; j--) {
              laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
                k] =
                laserCloudCornerArray[i + laserCloudWidth * (j - 1) + laserCloudWidth *
                  laserCloudHeight * k];
              laserCloudSurfArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
                k] =
                laserCloudSurfArray[i + laserCloudWidth * (j - 1) + laserCloudWidth *
                  laserCloudHeight * k];
            }
            laserCloudCornerArray[i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight *
              k] =
      ...
      for (int i = centerCubeI - halfCubeForMapping; i <= centerCubeI + halfCubeForMapping; i++) {
        for (int j = centerCubeJ - halfCubeForMapping; j <= centerCubeJ + halfCubeForMapping; j++) {
          for (int k = centerCubeK - halfCubeForMapping; k <= centerCubeK + halfCubeForMapping;
            k++)
          {
            ...
            {
              laserCloudValidInd[laserCloudValidNum] = i + laserCloudWidth * j + laserCloudWidth *
                laserCloudHeight * k;
              laserCloudValidNum++;
            ...
            if (pointSearchSqDis[4] < 1.0) {
              ...
              if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1]) {
                ...
                problem.AddResidualBlock(
                  ...
                  pose_graph_local.back().q.coeffs().data(), pose_graph_local.back().t.data());
                corner_num++;
              }
            }
          }
    ...
    MeasurementEdge measurement_edge(timeLaserOdometryLast, timeLaserOdometry, robot_id, robot_id,
      submap_id, submap_id, index - 1, index, q_last_curr_opt, t_last_curr_opt);
    ...
    br.sendTransform(
      tf::StampedTransform(
        transform, odomAftMapped.header.stamp, "/camera",
        "/aft_mapped"));
    ...
    if (frameCount % SUBMAP_LENGTH * 5 == 0 &&
      (t_base_curr.x() * t_base_curr.x() + t_base_curr.y() * t_base_curr.y() + t_base_curr.z() *
      t_base_curr.z() > 25))
    {
      if (DEBUG) {
        ROS_WARN_STREAM(
  ...
  void publishMap(const int & laserCloudSurroundNum = -1)
  {
    ...
    br.sendTransform(
      tf::StampedTransform(
        transform, ros::Time().fromSec(timeLaserOdometry),
        "/camera", "/submap" + s));
  ...
  void displayCloud(
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, const std::string & window_name,
    const std::string & id)
  {
    if (cloud->size() < 1) {
    ...
    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(new pcl::visualization::
      PCLVisualizer(window_name));
    pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI> point_cloud_color_handler(
      cloud, "intensity");
  ...
  void generateThumbnail(bool headless = false)
  {
    ...
    coeffs << coefficients->values[0], coefficients->values[1], coefficients->values[2],
      coefficients->values[3];
    for (int i = 0; i < c_building->points.size(); i++) {
      Eigen::Vector4f pt(c_building->points[i].x, c_building->points[i].y, c_building->points[i].z,
        1);
    ...
    for (int i = 0; i < c_building->points.size(); i++) {
      ...
      norm_point << building_normals->points[i].normal_x, building_normals->points[i].normal_y,
    ...
    if (NEED_CHECK_DIRECTION == 1) {
      ...
      if (DEBUG) {
        ROS_ERROR_STREAM(
          s << " z angle " << q_temp.angularDistance(
            Eigen::Quaternionf::Identity()) * 57.3);
      }
    ...
    *thumbnailCur = *z_aligned;         // save thumbnail point cloud
    ...
    // ROS_ERROR_STREAM("3 ");
    cv::Mat image(1.0 * (maxElementY - minElementY), 1.0 * (maxElementX - minElementX), CV_8UC3,
  ...
  void generateThumbnailForFrame(int fid)
  {
    ...
    imwrite(
      std::string(
        std::getenv(
          "HOME")) + "/gacm_output/pictures/eachframe/submap" + s + ".png", image);
