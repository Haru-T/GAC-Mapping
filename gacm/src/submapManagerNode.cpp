...
void cornerLastHandler(
  const sensor_msgs::PointCloud2ConstPtr & laserCloudCornerLast2)
{
  mBuf.lock();
  cornerLastBuf.push(laserCloudCornerLast2);
  ROS_INFO("cornerLastHandler(): size: %zu", cornerLastBuf.size());
  mBuf.unlock();
  is_first = false;
}

void surfLastHandler(
  const sensor_msgs::PointCloud2ConstPtr & laserCloudSurfLast2)
{
  mBuf.lock();
  surfLastBuf.push(laserCloudSurfLast2);
  ROS_INFO("surfLastHandler(): size: %zu", surfLastBuf.size());
  mBuf.unlock();
}
  ...
  const sensor_msgs::PointCloud2ConstPtr & laserCloudFullRes2)
{
  ...
  ROS_INFO("fullResHandler(): size: %zu", fullResBuf.size());
  mBuf.unlock();
}
...
void odometryHandler(const nav_msgs::Odometry::ConstPtr & laserOdometry)
{
  ...
  ROS_INFO("odometryHandler(): size: %zu", odometryBuf.size());
  mBuf.unlock();
}
...
void mapping_process()
{
  ...
  while (true) {
      ...
      !fullResBuf.empty() && !odometryBuf.empty())
    {
      ...
      ROS_DEBUG(
        "mapping_process(): timestamps: [%f, %f, %f, %f]",
        cornerLastBuf.front()->header.stamp.toSec(),
        surfLastBuf.front()->header.stamp.toSec(),
        fullResBuf.front()->header.stamp.toSec(),
        odometryBuf.front()->header.stamp.toSec());
      ...
      while (!odometryBuf.empty() &&
        odometryBuf.front()->header.stamp.toSec() < max_timestamp)
      {
        odometryBuf.pop();
      }
      if (odometryBuf.empty()) {
        ROS_WARN("mapping_process(): odometryBuf empty");
        mBuf.unlock();
        break;
      }

      while (!surfLastBuf.empty() &&
        surfLastBuf.front()->header.stamp.toSec() < max_timestamp)
      {
        surfLastBuf.pop();
      }
      if (surfLastBuf.empty()) {
        ROS_WARN("mapping_process(): surfLastBuf empty");
        mBuf.unlock();
        break;
      }

      while (!fullResBuf.empty() &&
        fullResBuf.front()->header.stamp.toSec() < max_timestamp)
      {
        fullResBuf.pop();
      }
      if (fullResBuf.empty()) {
        ROS_WARN("mapping_process(): fullResBuf empty");
        mBuf.unlock();
        break;
      }
      ...
      ROS_DEBUG(
        "mapping_process(): { max_timestamp: %f, timestamps: [%f, %f, "
        "%f, %f]}",
        max_timestamp, timeLaserCloudCornerLast, timeLaserCloudSurfLast,
      ...
      ROS_DEBUG("mapping_process(): messages popped");
      ...
      if (lastnode_created) {
        ...
        if (DEBUG) {
          ROS_INFO_STREAM(
            "\033[1;32mtest config id " << CONFIG_ID
                                        << "\033[0m");
        }
        // std_msgs::Empty empty;
        // pubNewSessionSignal.publish(empty);
      ...
      MapDatabase[cur_robot_id][cur_map_id]->process(
        cornerLastBuf.front(), surfLastBuf.front(), fullResBuf.front(),
        odometryBuf.front());
      ...
      if (MapDatabase[cur_robot_id][cur_map_id]->checkNeedNewSubMap()) {
        m_loop.lock();
        MapDatabase[cur_robot_id].push_back(
          MapDatabase[cur_robot_id][cur_map_id]
          ->createNewSubMap());       // check, is parameter enough?
        cur_map_id++;
        m_loop.unlock();
      }
  ...
  ROS_ERROR("mapping_process(): exit");
}

void testBaseScore(
  pcl::PointCloud<PointType>::Ptr all_submap_bases,
  std::vector<int> & knn_idx, std::vector<float> & knn_dist)
{
...
void testMatchCloud(
  ...
  pcl::PointCloud<PointType>::Ptr cloud_target, int ri_test,
  int si_test, const Eigen::Matrix4f & try_guess,
  float & final_score, Eigen::Matrix4f & final_guess)
{
...
void thumbnail_process()
{
  while (true) {
    ...
    if (cur_thumbnail_id < cur_map_id) {
      ...
      PointType temp_base;
      temp_base.x = (float)(MapDatabase[cur_robot_id][cur_thumbnail_id]
        ->t_world_base.x());
      temp_base.y = (float)(MapDatabase[cur_robot_id][cur_thumbnail_id]
        ->t_world_base.y());
      temp_base.z = (float)(MapDatabase[cur_robot_id][cur_thumbnail_id]
        ->t_world_base.z());
        ...
        *all_submap_bases += *submap_base_map[cur_robot_id];
        ...
        for (int i = 0; i < 1 && i < loop_scores.size(); i++) {
            ...
            loop_scores[i].angle_rel * 57.3 < 60)
          {
            ...
            testMatchCloud(
              ...
              loop_scores[i].try_guess, final_score, final_guess);
            ...
            loop_scores[i].angle_rel * 57.3 < 60))
          {
            ...
            if (icp3.hasConverged()) {
              ...
              if (final_score <
                ...
                (is_drone[cur_robot_id] && is_drone[ri_test] ?
                0.65 :
                0.75)))
              {
                ...
                file_pefix += "/gacm_output/cache/r" +
                  std::to_string(cur_robot_id) + "s" +
                  std::to_string(cur_loop_test_id) + "-r" +
                  std::to_string(ri_test) + "s" +
                  std::to_string(si_test) + "-";
                pcl::io::savePCDFileASCII(
                  file_pefix + "target.pcd",
                  *cloud_target);
                pcl::io::savePCDFileASCII(
                  file_pefix + "source.pcd",
                  *cloud_src);
                pcl::io::savePCDFileASCII(
        ...
        if (lastnode_created) {
          ...
          std::cout << "\033[1;32m\nCurrent robot[" << cur_robot_id
                    << "] end, ready to receive robot[" << cur_robot_id + 1
                    << "] data!\n\033[0m";
        }
      } else {
        if (is_first) {
        ...
        if (empty_loop >= 100000 && !lastnode_created) {
          ...
          CONFIG_ID++;
          if (DEBUG) {
            ROS_ERROR_STREAM("Config id " << CONFIG_ID);
          }
...
int main(int argc, char ** argv)
{
  ...
  MapDatabase[cur_robot_id].push_back(
    std::shared_ptr<SubMap>(new SubMap(nh)));   // first submap
  MapDatabase[cur_robot_id][cur_map_id]->initParameters(
    cur_robot_id, 0, NEED_CHECK_DIRECTION, 0);   // double check? // first node
  kdtree.reset(new pcl::KdTreeFLANN<PointType>());
  submap_base_map[cur_robot_id].reset(new pcl::PointCloud<PointType>());
  ...
  std::string data_dir1 = home_dir + "/gacm_output/data/testSavemap/fullcloud/",
    data_dir2 =
    home_dir + "/gacm_output/data/air_experiment/",               // deprecate
  ...
  // 删除相关文件夹
  // std::cout << "\033[1;36m\nneed to rm ${HOME}\\gacm_output folder? ( \"1\"
  // for yes, \"0\" for no) \n\033[0m"; int permission; cin >> permission;
  // if(permission == 1)
  // {
  ...
  if ((CACHE_PATH != cache_dir1) &&
    (dir = opendir(CACHE_PATH.c_str())) == NULL)
  {
  ...
  ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>(
    "/laser_odom_to_init", 100, odometryHandler);

  ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>(
    "/laser_full_3", 100, fullResHandler);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    pubOptimizedMap[i] =
      nh.advertise<sensor_msgs::PointCloud2>(map_topic.str(), 100);
  }
  pubRelEdge =
    nh.advertise<visualization_msgs::Marker>("/relative_observation", 100);
