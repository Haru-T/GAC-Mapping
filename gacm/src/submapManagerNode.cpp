...
public:
  ...
  : loop_score(loop_score_), angle_rel(angle_rel_), robot_id(robot_id_), submap_id(submap_id_),
    guess(guess_), try_guess(try_guess_) {}
};
...
void save_all()
{
  ...
  printf("\033[1;32m\nBegin save map\n\033[0m");
  if (DEBUG) {
    std::cout << "Current Edges " << loopEdgeBuf.size() << " : " << dropEdgeBuf.size() << "\n";
  }
  ...
  for (int i = 0; i < database_size; i++) {
    MapDatabase[cur_robot_id][i]->saveMapToFile(
      std::string(
        std::getenv(
          "HOME")) + "/gacm_output/data/testSavemap/");
  }
  std::fstream fs;
  // 打开并清空文件
  fs.open(
    std::string(std::getenv("HOME")) + "/gacm_output/data/testSavemap/robot" +
    std::to_string(cur_robot_id) + "_loop.cfg", std::ios::ate | std::ios::out);
  fs << cur_robot_id << "\n";
...
// 该函数只在main函数开始时进行
void load_all(
  std::string basepath = std::string(std::getenv(
    "HOME")) + "/gacm_output/data/testSavemap/")
{
  ...
  if (cur_robot_id == 0) {
    ...
    std::cin >> load;
    if (load != 1) {
      cout << "\033[1;32m\nload all robot done! Ready to receive robot[" << cur_robot_id <<
        "] data!\n\033[0m";
      return;
    }
  }
  ...
  for (int i = 0; i < database_size; i++) {
    ...
    MapDatabase[cur_robot_id].push_back(
      std::shared_ptr<SubMap>(
        new SubMap(
          basepath, cur_robot_id,
          i, is_drone[cur_robot_id], *nh_ref)));
  ...
  fs.close();
  if (DEBUG) {
    std::cout << "\nEdges loaded, Edges : " << acc_edge_cnt << ": " << adj_edge_cnt << ": " <<
      rej_edge_cnt << "\n";
  }
...
void pubLoopImage(cv::Mat & query_tn, cv::Mat & response_tn, bool is_accept)
{
  ...
  if (is_accept) {
    ...
    cv::copyMakeBorder(
      response_tn, response_tn, 5, 5, 5, 5, cv::BORDER_CONSTANT,
      cv::Scalar(0, 255, 0));
  } else {
  ...
  while (true) {
      ...
      !odometryBuf.empty())
    {
        ...
        timeLaserCloudFullRes != timeLaserOdometry)
      {
        if (DEBUG) {
          ROS_ERROR(
            "time corner %f surf %f full %f odom %f \n", timeLaserCloudCornerLast,
      ...
      MapDatabase[cur_robot_id][cur_map_id]->process(
        cornerLastBuf.front(),
        surfLastBuf.front(), fullResBuf.front(), odometryBuf.front());
...
void testBaseScore(
  pcl::PointCloud<PointType>::Ptr all_submap_bases, std::vector<int> & knn_idx,
  std::vector<float> & knn_dist)
{
  for (int knnid = 0; knnid < knn_idx.size(); knnid++) {
    ...
    if (ri == cur_robot_id && i >= cur_loop_test_id - 5) {
      if (DEBUG) {
        ROS_ERROR_STREAM("Skip test " << i << " (homo) too close to " << cur_loop_test_id);
      }
    ...
    float desc_score =
      ...
      MapDatabase[cur_robot_id][cur_loop_test_id]->descriptor_db[0].second).norm();
    if (DEBUG) {
      ROS_ERROR_STREAM(
        "Desc test " << ri << ":" << i << " and " << cur_robot_id << ":" << cur_loop_test_id << " dist " <<
          desc_score);
    }
    // diff_matrix(cur_loop_test_id, i) = desc_score;
    // base transform
    Eigen::Quaterniond q_rel = MapDatabase[cur_robot_id][cur_loop_test_id]->q_world_base.inverse() *
      MapDatabase[ri][i]->q_world_base;
    Eigen::Vector3d t_rel = MapDatabase[cur_robot_id][cur_loop_test_id]->q_world_base.inverse() *
    ...
    LoopScore loop_score(desc_score, (float)angle_rel, ri, i, init_guess /*update_guess*/,
      try_guess);
...
void testMatchCloud(
  ...
  pcl::PointCloud<PointType>::Ptr cloud_target, int ri_test, int si_test,
  const Eigen::Matrix4f & try_guess, float & final_score,
  Eigen::Matrix4f & final_guess)
{
  ...
  if (DEBUG) {
    ROS_INFO_STREAM(
      "init guess ndt score: " << cur_score << ", try guess score: " << cur_score_w << ", icp score: " <<
        icp2.getFitnessScore());
  }
  ...
  while (true) {
    ...
    if (cur_thumbnail_id < cur_map_id) {
      ...
      outfile.open(
        std::string(
          std::getenv(
            "HOME")) + "/gacm_output/timecost/thumbnail_time.txt", ios::app);
      ...
      if (netvlad_client.call(img_srv)) {
        ...
        MapDatabase[cur_robot_id][cur_thumbnail_id]->descriptor_db.push_back(
          std::make_pair(
            id_tn,
            desc));
      } else {
      ...
      outfile.open(
        std::string(
          std::getenv(
            "HOME")) + "/gacm_output/timecost/descriptor_time.txt", ios::app);
      outfile << (double)(end - start) / CLOCKS_PER_SEC << "\n";
      outfile.close();
        ...
        *all_submap_bases += *submap_base_map[cur_robot_id];
        ...
        for (int i = 0; i < 1 && i < loop_scores.size(); i++) {
            ...
            loop_scores[i].angle_rel * 57.3 < 60)
          {
            ...
            if (loop_scores.size() >= 3) {
              if (loop_scores[i].loop_score / loop_scores[loop_scores.size() - 1].loop_score >
                0.9)
              {
                break;
              }
            ...
            if (DEBUG) {
              ROS_ERROR_STREAM(
                "Homo Matching " << ri_test << ":" << si_test << " ; " << cur_loop_test_id << " with score " <<
                  loop_scores[i].loop_score << " with angle " << loop_scores[i].angle_rel * 57.3);
            }
            //visualization
            cv::Mat query_tn = cv::imread(
              MapDatabase[cur_robot_id][cur_loop_test_id]->thumbnails_db[0].second);
            cv::Mat response_tn =
              cv::imread(MapDatabase[ri_test][si_test]->thumbnails_db[0].second);
            ...
            Eigen::Matrix4f final_guess;
            testMatchCloud(
              cloud_src, cloud_target, ri_test, si_test, loop_scores[i].try_guess,
              final_score, final_guess);
            ...
            outfile1.open(
              std::string(
                std::getenv(
                  "HOME")) + "/gacm_output/timecost/single_match_homo.txt", ios::app);
            ...
            if (icp2.hasConverged()) {
              ...
              Eigen::Matrix3d R_rel =
                icp2.getFinalTransformation().topLeftCorner(3, 3).cast<double>();
              Eigen::Quaterniond q_rel;
              q_rel = R_rel;
              Eigen::Vector3d t_rel = icp2.getFinalTransformation().topRightCorner(
                3,
                1).cast<double>();

              MeasurementEdge measurement_edge(MapDatabase[cur_robot_id][cur_loop_test_id]->
                pose_graph_local[0].stamp,
              ...
              if (final_score < (is_drone[cur_robot_id] ? 0.3 : 0.5)) {
                ...
                cout << "\033[1;32m\nDone add edge homo! robot[" << ri_test << ":" << si_test <<
                  "] => robot[" << cur_robot_id << ":" << cur_loop_test_id << "]\n\033[0m\n";
              ...
              dropEdgeBuf.push_back(measurement_edge);
              if (DEBUG) {
                ROS_WARN_STREAM("drop edge converge(" << loop_scores[i].angle_rel * 57.3 << ")");
              }
              pubLoopImage(query_tn, response_tn, false);
            }
        ...
        for (int i = 0; i < 1 && i < loop_scores.size(); i++) {
          ...
          if (find_hetero_loop == false && loop_scores[i].loop_score < 0.9 &&
            (need_init_coord || is_drone[cur_robot_id] || loop_scores[i].angle_rel * 57.3 < 60 ))
          {
            // accept
            ...
            if (DEBUG) {
              ROS_ERROR_STREAM(
                "Hetero Matching " << ri_test << " : " << si_test << ";" << cur_loop_test_id << " with score " <<
                  loop_scores[i].loop_score << " with angle " << loop_scores[i].angle_rel * 57.3);
            }
            //visualization
            cv::Mat query_tn = cv::imread(
            ...
            outfile1.open(
              std::string(
                std::getenv(
                  "HOME")) + "/gacm_output/timecost/single_match_hetero.txt", ios::app);
            if ((double)(end_icp - start_icp) / CLOCKS_PER_SEC > 0.5) {
              outfile1 << (double)(end_icp - start_icp) / CLOCKS_PER_SEC << "\n";
            }
            outfile1.close();
            ...
            if (icp3.hasConverged()) {
              ...
              if (DEBUG) {
                ROS_INFO_STREAM(
                  "robot[" << ri_test << ":" << si_test << "] => robot[" << cur_robot_id << ":" << cur_loop_test_id << "] Fitness score final: " << final_score <<
                    "\n");
              }
              // save loop closure edge
              Eigen::Matrix3d R_rel =
              ...
              Eigen::Quaterniond q_rel;
              q_rel = R_rel;
              Eigen::Vector3d t_rel = icp3.getFinalTransformation().topRightCorner(
                3,
                1).cast<double>();
              ...
              if (final_score <
                (is_drone[cur_robot_id] &&
                need_init_coord ? (is_drone[ri_test] ? 0.8 : 0.9) : (is_drone[cur_robot_id] &&
                is_drone[ri_test] ? 0.65 : 0.75)))
              {
                ...
                std::cout << "\033[1;32m\nDone add edge hetero! robot[" << ri_test << ":" <<
                  si_test << "] => robot[" << cur_robot_id << ":" << cur_loop_test_id <<
                  "]\n\033[0m\n";
                need_init_coord = false;

                std::string file_pefix = std::getenv("HOME");
                file_pefix += "/gacm_output/cache/r" + std::to_string(cur_robot_id) + "s" +
                  std::to_string(cur_loop_test_id) + "-r" + std::to_string(ri_test) + "s" +
                  std::to_string(si_test) + "-";
            ...
            if (loop_scores[i].loop_score < 1.0) {
              ...
              if (DEBUG) {
                ROS_WARN_STREAM(
                  "(hetero loop)drop edge orientation(" << loop_scores[i].angle_rel * 57.3 << ")");
              }
              cv::Mat query_tn = cv::imread(
                MapDatabase[cur_robot_id][cur_loop_test_id]->thumbnails_db[0].second);
              cv::Mat response_tn = cv::imread(
        ...
        outfile.open(
          std::string(
            std::getenv(
              "HOME")) + "/gacm_output/timecost/loop_time.txt", ios::app);
        ...
        if (cur_loop_test_id > 1) {
          ...
          pcl::PointCloud<PointType>::Ptr cloud_target =
            MapDatabase[cur_robot_id][cur_loop_test_id]->getMapCloud();
          if (DEBUG) {
            ROS_INFO_STREAM("done fetch " << cloud_src->size() << " : " << cloud_target->size());
          }
          pcl::ApproximateVoxelGrid<PointType> avg;
          avg.setLeafSize(0.8f, 0.8f, 0.8f);                               // trade off
          // avg.setLeafSize(0.5f, 0.5f, 0.5f); // trade off
          ...
          outfile1.open(
            std::string(
              std::getenv(
                "HOME")) + "/gacm_output/timecost/single_match_adj.txt", ios::app);
          outfile1 << (double)(end_icp - start_icp) / CLOCKS_PER_SEC << "\n";
          outfile1.close();
        ...
        if (lastnode_created) {
          ...
          cout << "\033[1;32m\nCurrent robot[" << cur_robot_id <<
            "] end, ready to receive robot[" << cur_robot_id + 1 << "] data!\n\033[0m";
        }
      } else {
        if (is_first) {
          mTProcess.unlock();
        ...
        if (empty_loop >= 10000 && !lastnode_created) {
          ...
          MapDatabase[cur_robot_id].push_back(
            MapDatabase[cur_robot_id][cur_map_id]->createNewSubMap());                                                                 // check, is parameter enough?
          cur_map_id++;
...
void publish_opt_posegraph(bool pubmap = false)
{
  ...
  if (DEBUG) {
    std::cout << "Publish posegraph use " << (double)(end - start) / CLOCKS_PER_SEC << "\n";
  }
  updating_pg = false;
...
void publish_opt_map(bool pubmap = false, bool pubmerge = false)
{
  ...
  for (int r = 0; r <= cur_robot_id; r++) {
    ...
    if (pubmerge && laserCloudMap->points.size() > 100) {
      ...
      pcl::io::savePCDFileASCII(
        std::string(
          std::getenv(
            "HOME")) + "/gacm_output/data/testSavemap/fullcloud/" + ss + "full.pcd",
        *laserCloudMap);
      if (r < 3) {
        *mergeCloudMapG += *laserCloudMap;
        pcl::io::savePCDFileASCII(
          std::string(
            std::getenv(
              "HOME")) + "/gacm_output/data/testSavemap/fullcloud/mergeG.pcd", *mergeCloudMapG);
      } else {
        *mergeCloudMapA += *laserCloudMap;
        pcl::io::savePCDFileASCII(
          std::string(
            std::getenv(
              "HOME")) + "/gacm_output/data/testSavemap/fullcloud/mergeA.pcd", *mergeCloudMapA);
      }
      std::cout << "\nSave merge to pcd\n";
    }
  ...
  if (DEBUG) {
    std::cout << "Publish optimized map use " << (double)(end - start) / CLOCKS_PER_SEC << "\n";
  }
  updating_map = false;
}
...
void performOptimization()
{
  ...
  for (int ri = 0; ri <= cur_robot_id; ri++) {
    ...
    for (int i = 0; i < current_database_size; i++) {
      ...
      problem.AddParameterBlock(
        MapDatabase[ri][i]->q_world_base.coeffs().data(), 4, q_parameterization);
      ...
      for (int node_id = 0; node_id < current_node_size; node_id++) {
        problem.AddParameterBlock(
          MapDatabase[ri][i]->pose_graph_local[node_id].q.coeffs().data(), 4, q_parameterization);
      ...
      for (int edge_id = 0; edge_id < current_edge_size; edge_id++) {
        ceres::CostFunction * cost_function = PoseGraph3dErrorTerm::Create(
          MapDatabase[ri][i]->edges_local[edge_id].q, MapDatabase[ri][i]->edges_local[edge_id].t,
          cov);
        problem.AddResidualBlock(
          cost_function, NULL,
  ...
  for (int i = 0; i < cur_loop_size; i++) {
    ...
    problem.AddResidualBlock(
      cost_function, NULL, MapDatabase[r_id_f][s_id_f]->pose_graph_local[0].q.coeffs().data(),
  ...
  for (int ri = 1; ri <= cur_robot_id; ri++) {
    for (int si = 0; si < MapDatabase[ri].size() - 1; si++) {
      for (int is_homo = 0; is_homo <= 1; is_homo++) {
        ...
        for (int r = 0; r <= cur_robot_id; r++) {
          if (ri != r &&
            (is_homo == 1 ? is_drone[ri] == is_drone[r] : is_drone[ri] != is_drone[r]))
          {
        ...
        for (int knnid = 0; knnid < knn_idx.size(); knnid++) {
          ...
          Eigen::Quaterniond q_rel = MapDatabase[ri][si]->q_world_base.inverse() *
            MapDatabase[ri_test][si_test]->q_world_base;
          Eigen::Vector3d t_rel = MapDatabase[ri][si]->q_world_base.inverse() *
            (MapDatabase[ri_test][si_test]->t_world_base - MapDatabase[ri][si]->t_world_base);
          if (t_rel.norm() > 50) {
            continue;
          }
          Eigen::Matrix4f try_guess = Eigen::Matrix4f::Identity();
...
void checkCacheableMap()
{
  ...
  for (int ri = 0; ri <= cur_robot_id; ri++) {
    ...
    if (DEBUG) {
      cout << "\033[1;32m\nrobot[" << ri << "] submap num: " << MapDatabase[ri].size() <<
...
int main(int argc, char ** argv)
{
  ...
  ros::Subscriber subLaserCloudCornerLast = nh.subscribe<sensor_msgs::PointCloud2>(
    "/laser_cloud_corner_last", 100, cornerLastHandler);

  ros::Subscriber subLaserCloudSurfLast = nh.subscribe<sensor_msgs::PointCloud2>(
    "/laser_cloud_surf_last", 100, surfLastHandler);
