  ...
  if (access_status == -1) {
    ...
    if (DEBUG) {
      std::cout << "Load done, start pushing cloud with corner "
                << laserCloudMapCorner->points.size() << " and surf "
                << laserCloudMapSurf->points.size() << "\n";
    }
...
void SubMap::saveMapToFile(const std::string & map_path) const
{
  ...
  for (auto node : pose_graph_local) {
    fs << std::fixed << node.stamp << " " << node.frame_id << " " << node.q.w()
       << " " << node.q.x() << " " << node.q.y() << " " << node.q.z() << " "
...
void SubMap::loadMapFromFile(const std::string & map_path)
{
  ...
  if (DEBUG) {
    std::cout << " local pose graph size " << pose_graph_local_size << "\n";
  }
  ...
  edges_local.clear();
  if (DEBUG) {
    std::cout << " local edges size " << edges_local_size << "\n";
  }
...
void SubMap::allocateMemory()
{
  ...
  // last thumbnail
  // thumbnailLast.reset(new pcl::PointCloud<PointType>()); //initialize when
  // create new submap
  // cur thumbnail
  ...
  int64_t timestamp_ns)
{
  ...
  laserCloudCornerLast = std::move(laser_cloud_corner_last);
  laserCloudSurfLast = std::move(laser_cloud_surf_last);
  laserCloudFullRes = std::move(laser_full_3);

  if (timeLaserOdometryLast == 0 && !initWithPose) {
    q_world_base = orientation;
    t_world_base = position;
    q_odom_last = orientation;
    t_odom_last = position;
    initWithPose = true;
    return;
  }
  ...
  q_base_curr = q_odom_map * orientation;
  t_base_curr = q_odom_map * position + t_odom_map;
  ...
  nav_msgs::Odometry::ConstPtr odometryBufFrontPtr)
{
  ...
  // ROS_ERROR_STREAM("process begin " <<
  // timeLaserOdometry-timeLaserOdometryLast);
  if (timeLaserOdometry - timeLaserOdometryLast <= 0) {
    timeLaserOdometry = timeLaserOdometryLast;
  }
...
void SubMap::publishMap(int laserCloudSurroundNum)
{
  ...
  // ROS_WARN_STREAM("Publish submap " << s << " at " <<std::setprecision(20) <<
  // timeLaserOdometry);
...
void SubMap::generateThumbnail(bool headless)
{
  ...
  if (DEBUG) {
    ROS_ERROR_STREAM("done transform");
  }
  ...
  *thumbnailCur = *z_aligned; // save thumbnail point cloud
  if (DEBUG) {
    ROS_ERROR_STREAM("done check direction");
  }
  ...
  if (!headless) {
    if (DEBUG) {
      ROS_ERROR_STREAM("wait display thumbnail");
    }
  ...
  if (DEBUG) {
    ROS_ERROR_STREAM("done generate thumbnail");
  }
}
...
void SubMap::generateThumbnailForFrame(int fid)
{
  ...
  transform_temp.topLeftCorner(3, 3) = q_temp.toRotationMatrix();
  pcl::PointCloud<PointType>::Ptr z_aligned(new pcl::PointCloud<PointType>());
  // pcl::PointCloud<PointType>::Ptr z_aligned_plane (new
